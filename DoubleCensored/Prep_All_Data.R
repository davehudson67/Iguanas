...
#Allig <- read_csv("DoubleCensored/All_Island_Data/Cfigginsi_Alligatory_Cay.csv")
Bitter <- read_csv("DoubleCensored/All_Island_Data/BitterGuanaCay_clean.csv") %>%
  mutate(date = dmy(date)) %>%
  mutate(feeding = as.factor(feeding))
summary(Bitter$feeding)
FFRC <- read_csv("DoubleCensored/All_Island_Data/FlatRockThru2019_CleanOct2020.csv")%>%
  mutate(date = mdy(date))  %>%
  mutate(feeding = as.factor(feeding))
summary(FFRC$feeding)
Gaulin <- read_csv("DoubleCensored/All_Island_Data/GaulinCay_clean.csv") %>%
  mutate(date = dmy(date)) %>%
...
#Leaf_A <- read_csv("DoubleCensored/All_Island_Data/Cfigginsi_Leaf_Cay_Allens.csv")
Leaf <- read_csv("DoubleCensored/All_Island_Data/LeafCayThru2019_CleanOct2020.csv") %>%
  mutate(date = mdy(date))  %>%
  mutate(feeding = as.factor(feeding))
summary(Leaf$feeding)
Noddy <- read_csv("DoubleCensored/All_Island_Data/NoddyCCay_Clean.csv")%>%
  mutate(date = dmy(date)) %>%
  mutate(feeding = as.factor(feeding))
summary(Noddy$feeding)
NAdder <- read_csv("DoubleCensored/All_Island_Data/NorthAdderly_clean.csv")%>%
  mutate(date = dmy(date))  %>%
  mutate(feeding = as.factor(feeding))
summary(NAdder$feeding)
UCay <- read_csv("DoubleCensored/All_Island_Data/UcayThru2019_CleanOct2020.csv") %>%
  mutate(date = mdy(date))  %>%
  mutate(feeding = as.factor(feeding))
summary(UCay$feeding)
WhiteB <- read_csv("DoubleCensored/All_Island_Data/WhiteBay_clean.csv") %>%
  mutate(date = dmy(date))  %>%
  mutate(feeding = as.factor(feeding))
...
## Prepare data
first_year <- min(AllData$Year) - 30
...
rm(NACols)
rm(tag_index)
save.image("igs_AllIslands_CleanDH_190625.RData")
