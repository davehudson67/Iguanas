...
code <- nimbleCode({
  for (i in 1:nind) {
    ...
    # Calculate raw discrete index based on tB[i]. 
    t_discrete_idx_raw[i] <- floor(tB[i] - t_first_RW) + 1
    
    # Clamp the index to the valid range [1, n_year].
    # Birth times before the first bin use u_a_center[1].
    # Birth times after the last bin use u_a_center[n_year].
    t_idx_clamped[i] <- max(1, min(n_year, t_discrete_idx_raw[i]))
    
    # Select the RW effect for the cohort (birth year).
    u_a_cohort[i] <- u_a_center[t_idx_clamped[i]]
    
    ## Linear predictors with species random effect (using binned effect)
    log(amult[i]) <- log(a) +
      betaSEX[1]  * sex[i]      * zSEX[1] +
      betaFEED[1] * feeding[i] * zFEED[1] +
      betaSPECIES[1] * species[i] * zSPECIES[1] +
      u_a_cohort[i] # <-- SIMPLIFIED DISCRETE COHORT EFFECT
...
n_year <- max(year_index) - min(year_index)
...
config <- configureMCMC(model, monitors = c("a", "b", "mean.p", "betaSEX", "betaFEED", "betaSPECIES",
                                            "zSEX", "zFEED", "zSPECIES", "sigma_year_a", "u_a_center"))
config$removeSamplers(c("a", "b"))
config$addSampler(target = c("a", "b"), type = 'AF_slice')

## load in custom RJ-MCMC samplers (Source file not provided, assumed to exist)
