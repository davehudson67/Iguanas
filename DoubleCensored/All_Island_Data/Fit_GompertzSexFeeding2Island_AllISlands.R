...
feeding <- AllData %>%
  ...
  mutate(feeding = ifelse(feeding != "none", 1, 0)) %>%
  distinct(ID, .keep_all = TRUE) %>%
  pull(feeding)
...
consts <- list(nind = nind, tMax = ncol(CH), sex = sex, island = island, nISLANDS = max(island), feeding = feeding)
...
## configure MCMC
#config <- configureMCMC(model, monitors = c("a", "b", "sigmaA", "sigmaB", "mean.p", "betaSEX", "betaFEEDmid", "betaFEEDno", "zSEX", "zFEED"))
config <- configureMCMC(model, monitors = c("a", "b", "sigmaA", "sigmaB", "mean.p", "betaSEX", "betaFEED", "zSEX", "zFEED"))
