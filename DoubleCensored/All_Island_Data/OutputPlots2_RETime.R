...
run <- readRDS("DoubleCensored/All_Island_Data/Samples/G_AllIslands_SexFeeding2_RETime_a.rds")
...
samples <- do.call(rbind, run)

## Marginal probabilities of inclusion for each variable
zNames <- c('zSEX[1]', 'zSEX[2]', 'zFEED[1]', 'zFEED[2]')
...
################################################################################
#
#  Posterior Desnity Plots of effects
#
################################################################################
...
colnames(postdf)
colnames(postdf) <- make.unique(gsub("\\[.*\\]", "", colnames(postdf)))

postdf <- select(postdf, c(1:6, 49:52)) %>%
    rename(betaFEEDa = betaFEED,
           betaFEEDb = betaFEED.1,
           betaSEXa     = betaSEX,
           betaSEXb     = betaSEX.1,
           zFEEDa       = zFEED,
           zFEEDb       = zFEED.1,
...
## Combine dfs
betaFEED <- bind_rows(betaFEED_a, betaFEED_b)

# Compute 95% credible intervals for each side
ci_a <- ci(betaFEED_a$Estimate, method = "HDI", ci = 0.95)
ci_b <- ci(betaFEED_b$Estimate, method = "HDI", ci = 0.95)

ci_df <- data.frame(
  parameter = c("a", "b"),
  variable = rep("Some feeding", 2),
  CI_low = c(ci_a$CI_low, ci_b$CI_low),
  CI_high = c(ci_a$CI_high, ci_b$CI_high)
)
