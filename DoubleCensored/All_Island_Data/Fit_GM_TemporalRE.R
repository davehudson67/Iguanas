...
## configure MCMC
config <- configureMCMC(model, monitors = c("a", "b", "mean.p", "betaSEX", "betaFEEDmid", "betaFEEDno", 
...
monitors <- c("a","b","mean.p","betaSEX","betaFEEDmid","betaFEEDno",
              "zSEX","zFEED","u_a","sigma_year_a")
...
workflow <- function(seed, dat, consts, monitors, rj_path, gompz_path1, gompz_path2, gompz_pathN) {
  ...
  initFn <- function(model, dat, consts) {
    repeat {
      ...
      inits$betaFEEDmid <- rnorm(2)
      inits$betaFEEDno <- rnorm(2)
      inits$zSEX <- c(1,1)
      inits$zFEED <- c(1,1)
...
saveRDS(samples, "DoubleCensored/All_Island_Data/Samples/G_AllIslands_SexFeeding3_RETime_a.rds")
