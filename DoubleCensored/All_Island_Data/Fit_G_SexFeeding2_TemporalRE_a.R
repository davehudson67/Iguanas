  ...
  load("igs_AllIslands_CleanDH_290825_obsStart.RData")
  ...
  #tMax = ncol(CH)
  #sex = sex[1:100]
  #feeding = feeding[1:100]
  #year_index = year_index[1:100]
  
  #y = y[1:100]
  #cintB = cintB[1:100,]
  #cintD = cintD[1:100,]
  #censoredD = censoredD[1:100]
  #tD = tD[1:100]
  #tB = tB[1:100]
  #tstar = tstar[1:100]
  #dind = dind[1:100]
  ...
  code <- nimbleCode({
    for (i in 1:nind) {
      ...
      tstar[i] ~ dGompertzLB(amult[i], bmult[i], lowerBound = L[i])   # <- CHANGED
  ...
  initFn <- function(model, cintB, cintD, censoredD, n_year) {
    ...
    repeat {
      ...
      inits <- list(
        ...
        betaSEX = rnorm(2), betaFEED = rnorm(2),
        u_a = rep(0, n_year), sigma_year_a = 0.5,
        zSEX = c(0,0), zFEED = c(0,0)
      )
...
workflow <- function(seed, dat, consts, monitors, rj_path, gompz_path1, gompz_path2, gompz_pathN) {
  ...
  initFn <- function(model, cintB, cintD, censoredD, n_year) {
    repeat {
      ...
      if (length(id_cens)) {
        tDinit[id_cens] <- cintD[id_cens, 2] + rexp(length(id_cens), rate = 5)
      }
      if (length(id_int)) {
        tDinit[id_int] <- runif(length(id_int), cintD[id_int, 1], cintD[id_int, 2])
      }
  ...
  inits <- initFn(cModel, dat$cintB, dat$cintD, dat$censoredD, consts$n_year)
  ...
  set.seed(seed)
  run <- runMCMC(cmcmc,
                 niter = 250000, nburnin = 22000, thin = 1,
...
seeds <- c(2022, 653)  # Add more seeds if you want more chains
