...
cintB <- cbind(tF - 1, tF )
cintB[is.na(tKB), 1] <- tF[is.na(tKB)] - 15
cintB[cintB <= 0] <- 1
#cintB[is.na(tKB), 2] <- 0
colnames(cintB) <- NULL
...
saveRDS(run, "Samples/G_dc_LeafCay.rds")
run <- readRDS("Samples/G_dc_LeafCay.rds")
run$summary
...
loglike <- function(censoredB, bL, bU, censoredD, tL, dU, y, tMax, a, b, p) {
  
  ## loop over individuals
  fsurv1 <- pmap_dbl(list(censoredB, bL, bU, censoredD, tL, dU, y, tMax), function(censoredB, bL, bU, censoredD, tL, dU, y, tMax, a, b, p) {
#browser()
    ll <- y * log(p)
    ...
    if (censoredD == 1) { #interval censored
      ...
      temp <- sapply(bL:bU, function(tB_val) {
        pGompertz(t1 - tB_val, a, b) - pGompertz(t1 - 1 - tB_val, a, b)
      })
      death_ll <- log(temp)
      ...
      if (tL < tMax) {
        ...
        temp_tail <- sapply(bL:bU, function(tB_val) {
          pGompertz(tMax - tB_val, a, b, lower.tail = FALSE)
        })
...
logimpweight <- mclapply(logimpweight,
                         function(pars, censoredB, bL, bU, censoredD, tL, dU, y, tMax) {
                           loglike(censoredB, bL, bU, censoredD, tL, dU, y, tMax, pars[1], pars[2], pars[3])
...
saveRDS(logimpweight, "LImpWeights/logimpweight_g.rds")
logimpweight <- readRDS("LImpWeights/logimpweight_g.rds")
