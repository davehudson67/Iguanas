...
source("Distributions/Dist_SilerLB.R")
...
code <- nimbleCode({
  ...
  for (i in 1:nind) {
    ...
    tD[i] ~ dSilerLB(a1, a2, b1, b2, c, tB[i])
...
config <- configureMCMC(cModel, monitors = c("a1", "a2", "b1", "b2", "c", "mean.p", "tB", "tD"), thin = 1)
...
saveRDS(run, "Samples/S_dc_LeafCay.rds")
run <- readRDS("Samples/S_dc_LeafCay.rds")
run$summary

mcmcplot(run$samples, parms = c("a1", "a2", "b1", "b2", "c", "mean.p"))
...
colnames(props) <- c("a1", "a2", "b1", "b2", "c", "mean.p")
...
defense <- defense %>%
  mutate(mean.p = dmp)
colnames(defense) <- c("a1", "a2", "b1", "b2", "c", "mean.p")
...
loglike <- function(censoredB, bL, bU, censoredD, tL, dU, y, tMax, a1, a2, b1, b2, c1, p) {
  
  ## loop over individuals
  fsurv1 <- pmap_dbl(list(censoredB, bL, bU, censoredD, tL, dU, y, tMax), function(censoredB, bL, bU, censoredD, tL, dU, y, tMax, a1, a2, b1, b2, c1, p) {
    ...
    if (censoredD == 1) { #interval censored
      ...
      temp <- sapply(bL:bU, function(tB_val) {
        pSiler(t1 - tB_val, a1, a2, b1, b2, c1) - pSiler(t1 - 1 - tB_val, a1, a2, b1, b2, c1)
      })
      death_ll <- log(temp)
      ...
      if (tL < tMax) {
        ...
        temp_tail <- sapply(bL:bU, function(tB_val) {
          pSiler(tMax - tB_val, a1, a2, b1, b2, c1, lower.tail = FALSE)
        })
...
logimpweight <- mclapply(logimpweight,
                         function(pars, censoredB, bL, bU, censoredD, tL, dU, y, tMax) {
                           loglike(censoredB, bL, bU, censoredD, tL, dU, y, tMax, pars[1], pars[2], pars[3], pars[4], pars[5], pars[6])
...
saveRDS(logimpweight, "LImpWeights/logimpweight_s.rds")
logimpweight <- readRDS("LImpWeights/logimpweight_s.rds")
